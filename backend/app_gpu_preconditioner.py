...
class ReservoirGrid:
    def __init__(self, nx, ny, dx, dy, phi, k_matrix, k_frac_map, ct, mu, p_init=3.5e7):
        ...
        self.ct = ct
        self.mu = mu
        # Build permeability field; override with provided fracture permeability values.
        self.k = np.ones((nx, ny), dtype=float) * k_matrix
        for (ix, iy), kf in k_frac_map.items():
            self.k[ix, iy] = kf
        # Set uniform initial pressure.
...
###############################################################################
# 3. LinearOperator-based Jacobi Preconditioner
###############################################################################
from cupyx.scipy.sparse.linalg._interface import LinearOperator

class JacobiPreconditioner(LinearOperator):
    """
    A simple diagonal (Jacobi) preconditioner as a LinearOperator.
    It stores the inverse of the diagonal of A_gpu.
    """
    def __init__(self, A_gpu):
        shape = A_gpu.shape
        dtype = A_gpu.dtype
        super().__init__(dtype=dtype, shape=shape)
        diag_entries = A_gpu.diagonal()  # On GPU
        self.inv_diag = 1.0 / diag_entries

    def _matvec(self, x):
        return self.inv_diag * x

###############################################################################
# 4. Fracture Source Terms and Pressure Solve with Preconditioner
###############################################################################
...
def solve_pressure_implicit_gpu(grid, T_gpu, dt, fractures, prod_rate, use_preconditioner=True):
    """
    Assemble A = M + T, then solve A x = b using GMRES with an optional Jacobi preconditioner.
    """
    nx, ny = grid.nx, grid.ny
    ...
    # Apply Dirichlet BC modifications
    ...
    if use_preconditioner:
        M_precond = JacobiPreconditioner(A_gpu)
...
def run_simulation(
    ...
    # Check input grid size to avoid excessive memory usage.
    MAX_CELLS = 1e7  # For example, limit to 10 million cells
    if nx * ny > MAX_CELLS:
        raise ValueError(f"Grid size too large: {nx*ny} cells; maximum allowed is {int(MAX_CELLS)} cells.")
    ...
    # Build transmissibility matrix on GPU
    t_T_start = time.time()
    ...
    perf['simulation_loop_total_time_gpu'] = time.time() - t_sim_start
    ...
    time_complexity = {
        ...
        "solve_pressure_implicit_gpu": "GMRES with optional Jacobi preconditioning: O(iterations * nnz)",
        "simulation_loop": f"{n_steps} timesteps"
    }
    return grid, fractures, times_list, None, perf, time_complexity

###############################################################################
# 6. Flask App: Endpoints and Output Files (GPU)
###############################################################################
app = Flask(__name__)
CORS(app)

RESULT_FILENAME = os.path.join(app.root_path, "simulation_results.json")
PERFORMANCE_LOG_FILENAME = os.path.join(app.root_path, "gpu_performance_log.json")
...
def run_simulation_endpoint():
    print("Running Simulation (GPU version) with Jacobi preconditioner using GMRES...")
    nx, ny, dx, dy = 900, 900, 10.0, 10.0
    ...
    try:
        grid, fracs, times, pressures, perf, complexity = run_simulation(
            ...
            frac_bc_fraction=0.8,
            use_preconditioner=True
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 400

    # Prepare simulation results for the viewer.
    simulation_data = {
        "nx": nx,
        "ny": ny,
        "dx": dx,
        ...
        "well_p": 1e5,
        "timeSteps": times,
        "notes": "GPU-based simulation using GMRES with a simple Jacobi preconditioner."
    }
    ...
    with open(PERFORMANCE_LOG_FILENAME, "w") as f:
        json.dump({**perf, "time_complexity": complexity}, f)
    print("Performance log saved to", PERFORMANCE_LOG_FILENAME)
    
    return jsonify({"message": "Simulation run (GPU version with GMRES and preconditioning) completed and data saved."})
